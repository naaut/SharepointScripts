...
if ($env:COMPUTERNAME -ne $newname) {{
 ...
 if (($env:COMPUTERNAME -eq $newname) -and ($curentdomain -eq $domainname) -and ($env:USERDNSDOMAIN -ne $curentdomain)) {{
    ...
    InlineScript {{C:\SharePoint\PrerequisiteInstaller.exe /continue /unattended | Out-Null}}
    Restart-Computer -Wait -Force
    #InlineScript {{C:\SharePoint\PrerequisiteInstaller.exe /continue /unattended; Start-Sleep 240}}
...
#config logon as sp-installer and restart
 ...
 Restart-Computer -Force #> 
    ...
    Start-Process powershell.exe -Credential $cred -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Normal -NoProfile -NoExit -Command C:\Scripts\sp-install-frontend.ps1 -domain '$Using:domainname' -pass '$Using:domainpass'  -passphrase 'eeM8oo41'"
    #Start-Process powershell.exe -Credential $cred -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Normal -NoProfile -NoExit -Command C:\Scripts\Create-FrontEndSite.ps1 -domain $Using:domainname"
    }}
...
if (!(Test-Path "c:\Scripts\config.xml")){{
    ...
    Invoke-WebRequest $source -OutFile $destination
    $source = "http://cubes-scripts.s3.amazonaws.com/sp-install-frontend.ps1"
    $destination = "c:\Scripts\sp-install-frontend.ps1" 
    Invoke-WebRequest $source -OutFile $destination
    $source = "http://cubes-scripts.s3.amazonaws.com/Create-FrontEndSite.ps1"
    $destination = "c:\Scripts\Create-FrontEndSite.ps1" 
    Invoke-WebRequest $source -OutFile $destination
}}
