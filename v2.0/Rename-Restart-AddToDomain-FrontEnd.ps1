...
if ($env:COMPUTERNAME -ne $newname) {{
 ...
 if (($env:COMPUTERNAME -eq $newname) -and ($curentdomain -eq $domainname) -and ($env:USERDNSDOMAIN -ne $curentdomain)) {{
    ...
    Start-Process powershell.exe -Credential $cred -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Normal -NoProfile -NoExit -Command C:\Scripts\sp-install-frontend.ps1 -domain '$Using:domainname' -pass '$Using:domainpass'  -passphrase 'eeM8oo41' -SQLName '$Using:SQLName'"
    }}
