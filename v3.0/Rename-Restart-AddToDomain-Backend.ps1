...
if ($env:COMPUTERNAME -ne $newname) {{
  Rename-Computer -Newname $newname -Force -Passthru
      ...
      Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"  | Out-File "C:\Logs\Backend.log" -Append
      Write-Output "`nRename Computer Start..." | Out-File "C:\Logs\Backend.log" -Append
      Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::" | Out-File "C:\Logs\Backend.log" -Append
 ...
 if ($curentdomain -ne $domainname){{
    InlineScript {{
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"  | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`nAdd to Domain Computer Start..." | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::" | Out-File "C:\Logs\Backend.log" -Append  
    ...
    try {{
        ...
        Write-Output "`nWait DC...." | Out-File "C:\Logs\Backend.log" -Append 
        Start-Sleep 30
        ipconfig /flushdns
 ...
 InlineScript {{ 
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"  | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`nAdd sp-installer and sp-farm to local Administrators group..." | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::" | Out-File "C:\Logs\Backend.log" -Append  
 ...
 if (($env:COMPUTERNAME -eq $newname) -and ($curentdomain -eq $domainname) -and ($env:USERDNSDOMAIN -ne $curentdomain)) {{
    ...
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"  | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`nStart External Script for promotion BackEnd Server ..." | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::" | Out-File "C:\Logs\Backend.log" -Append 

    $password = ConvertTo-SecureString $Using:domainpass -AsPlainText -Force
    $cred = new-object -typename System.Management.Automation.PSCredential -argumentlist "$Using:netbiosname\sp-installer", $password
    Start-Process powershell.exe -Credential $cred -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Normal -NoProfile -NoExit -Command C:\Scripts\sp-install-backend.ps1 -domain $Using:domainname -pass '$Using:domainpass'  -passphrase 'eeM8oo41' -SQLName '$Using:SQLName01'"
    }}
             
 }}
    InlineScript {{
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"  | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`nWorkflow completed... please wait external script..."  | Out-File "C:\Logs\Backend.log" -Append
    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::" | Out-File "C:\Logs\Backend.log" -Append
    }}
...
Start-Transcript -Path "c:\Logs\Backend.log" -Append
...
if (!(Test-Path "c:\Scripts\sp-install-backend.ps1")){{    Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"
    ...
    $source = "http://cubes-scripts.s3.amazonaws.com/v3.0/sp-install-backend.ps1"    $destination = "c:\Scripts\sp-install-backend.ps1"     Invoke-WebRequest $source -OutFile $destination
    $source = "http://cubes-scripts.s3.amazonaws.com/v3.0/create-shtask.ps1"    $destination = "c:\Scripts\create-shtask.ps1"     Invoke-WebRequest $source -OutFile $destination
