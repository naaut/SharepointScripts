...
if ($curentdomain -ne $domainname)
    {{
    $logfolder = "c:\Logs\Backend.log"
    }}
else
    {{    
    $logfolder = "\\$($NIC.DNSServerSearchOrder | Select -First 1)\Logs\Backend.log"
    }}
...
if ($curentdomain -ne $domainname)
    ...
    if (!(Test-Path "c:\Scripts\sp-install-backend.ps1")){{        Write-Output "`n:::::::::::::::::::::::::::::::::::::::::::::::::::::"
        ...
        $source = "http://cubes-scripts.s3.amazonaws.com/v3.1/sp-install-backend.ps1"        $destination = "c:\Scripts\sp-install-backend.ps1"         Invoke-WebRequest $source -OutFile $destination
   ...
   if (!(Test-Path "\\$($NIC.DNSServerSearchOrder | Select -First 1)\Logs\Backend.log")) 
       {{
       
       Write-Output "`n::::"   |  Out-File "C:\Logs\Backend.log" -Append default
       Write-Output "`nCopy log file to DC..."   |  Out-File "C:\Logs\Backend.log" -Append default
       Write-Output "`n::::"  |  Out-File "C:\Logs\Backend.log" -Append default
       
       Copy-Item -Path "C:\Logs\Backend.log" -Destination "\\$($NIC.DNSServerSearchOrder | Select -First 1)\Logs\"

       Write-Output "`n::::"  |  Out-File "\\$($NIC.DNSServerSearchOrder | Select -First 1)\Logs\Backend.log" -Append default
       Write-Output "`nFile copied..."  |  Out-File "\\$($NIC.DNSServerSearchOrder | Select -First 1)\Logs\Backend.log" -Append default
       Write-Output "`n::::"  |  Out-File "\\$($NIC.DNSServerSearchOrder | Select -First 1)\Logs\Backend.log" -Append default
       }}
   }}
